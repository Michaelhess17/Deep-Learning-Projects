...
# File: final.py
# Name: Michael Hess
# Description: Board AI + Player AI
import random
import copy 
...
class Board:
	...
	def playGame(self, px, po):
		...
		if px == 'HUMAN':
			while self.isFull() != True:
				humanMove = int(input("X's Move"))
				if not self.allowsMove(humanMove):
					print("That is not a valid move")
					humanMove = int(input("X's Move"))
					self.addMove(humanMove, px_token)
				else:
					self.addMove(humanMove, px_token)
				print(self)
				if self.winsFor('X'):
					print('The human humbly wins! XxXxXx')
					break
				self.addMove(po.nextMove(self), po_token)
				print(self)
				if self.winsFor('O'):
					print('THE COMPUTER BEAT THE MASTER!!! OoOoOo')
					break
				if self.isFull():
					print('It\'s a tie!')
		elif po == 'HUMAN':
			while self.isFull() != True:
				self.addMove(px.nextMove(self), px_token)
				print(self)
				if self.winsFor('X'):
					print('THE COMPUTER BEAT THE MASTER!!! XxXxXx')
				humanMove = int(input("O's Move"))
				if not self.allowsMove(humanMove):
					print("That is not a valid move")
					humanMove = int(input("O's Move"))
					self.addMove(humanMove, po_token)
				else:
					self.addMove(humanMove, po_token)
				print(self)
				if self.winsFor('O'):
	...
	def playGame2(self, px, po):
		px_token = 'X'
		po_token = 'O'
		if px == 'HUMAN':
			while self.isFull() != True:
				randNum = random.randint(1, self.width - 1)
				humanMove = int(input("X's Move"))
				if random.randint(1, self.width - 1) == randNum and self.allowsMove(randNum):
					humanMove = randNum
				if not self.allowsMove(humanMove):
					print("That is not a valid move")
					humanMove = int(input("X's Move"))
				...
				if self.winsFor('X'):
					print('The human humbly wins! XxXxXx')
					break
				compMove = po.nextMove(self)
				if random.randint(1, self.width - 1) == randNum and self.allowsMove(randNum):
					compMove = randNum
				self.addMove(compMove, po_token)
				print(self)
				if self.winsFor('O'):
					print('THE COMPUTER BEAT THE MASTER!!! OoOoOo')
					break
			...
			while self.isFull() != True:
				randNum = random.randint(1, self.width - 1)
				compMove = px.nextMove(self)
				if random.randint(1, self.width - 1) == randNum and self.allowsMove(randNum):
					compMove = randNum
				self.addMove(compMove, px_token)
				print(self)
				if self.winsFor('X'):
					print('THE COMPUTER BEAT THE MASTER!!! XxXxXx')
				humanMove = int(input("O's Move"))
			...
			while self.isFull() != True:
				randNum = random.randint(1, self.width - 1)
				xMove = px.nextMove(self)
				if random.randint(1, self.width - 1) == randNum and self.allowsMove(randNum):
					xMove = randNum
				self.addMove(xMove, px_token)
				print(self)
				if self.winsFor('X'):
					print('X Wins! XxXxXx')
					break
	...
	def scoresFor(self, b):
		...
		for column in range(b.width):
				if not b.allowsMove(column):
					...
					b.addMove(column, self.ox)
					op = Player(self.oppCh(), self.tbt, self.ply - 1)
					oppScore = op.scoresFor(b)
					scores[column] = 100.0 - max(oppScore)
					b.delMove(column)
		return scores

	def nextMove(self, b):
		scores = self.scoresFor(b)
		tbMove = self.tiebreakMove(scores)
		return tbMove
